...
##############################################################################
def evaluate_prolog_generation(model, tokenizer, dataset, max_new_tokens=1024):
    ...
    # Initialize WandB table for results
    ...
    for idx, sample in enumerate(tqdm(dataset, desc="Evaluating")):
        ...
        try:
            ...
            attempt_info = {
                ...
                'extracted_code': prolog_code if prolog_code else "No code extracted",
            ...
            if not prolog_code:
                ...
                attempt_info['structure_valid'] = check_structure_correctness(prolog_code)

                # Check if execution yielded a number and if it's correct
                try:
                    if final_line:
                        float_result = float(final_line)
                    ...
                    print(f"Sample {sample_index}: Prolog code did not yield a numeric result ('{final_line}').")
                except Exception as e:
                    attempt_info['reason_for_failure'] = f"Error checking result: {str(e)}"
                    print(f"Sample {sample_index}: Error checking result: {str(e)}")

            # (5) Validate numeric correctness (for overall sample metrics)
            valid_start = time.time()
            is_strict = False
            is_arithmetic = False
            if success and gold_val is not None:
                try:
                ...
                if not is_strict and ARITHMETIC_PATTERN.match(final_line.strip()):
                    try:
                        eval_val = float(final_line.strip())
            ...
            # Create a placeholder attempt_info for error cases
            attempt_info = {
                'model_output': 'ERROR',
                ...
                'structure_valid': False,
                'generation_time': 0,
                'prolog_execution_time': 0
            }

        # --- Log results to WandB Table ---
        results_table.add_data(
            ...
            str(attempt_info['execution_result']) if attempt_info['execution_result'] is not None else "",
            ...
            f"{attempt_info['prolog_execution_time']:.3f}" if attempt_info['prolog_execution_time'] is not None else "",
            str(attempt_info['reason_for_failure']) if attempt_info['reason_for_failure'] is not None else ""
        )
