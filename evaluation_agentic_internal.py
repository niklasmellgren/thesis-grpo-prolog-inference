...
def run_prolog(code: str, timeout: int = 5) -> str:
    ...
    # Add solve/1 predicate if not present
...
# ─── Revised agentic_loop without seed ─────────────────────────────────
def agentic_loop(model, system_prompt, user_query, max_steps=20):
...
def evaluate_agentic_prolog(model, dataset, max_steps: int = 8):
    ...
    for idx, sample in enumerate(tqdm(dataset, desc="Evaluating"), start=1):
        ...
        gen_t = time.time() - t0
        stats["gtimes"].append(gen_t)  # ADD THIS LINE
    ...
    final = {
        "final/prolog_accuracy":   stats["strict"]/stats["total"]*100 if stats["total"] else 0, # Handle ZeroDivisionError
        "final/structure_accuracy":stats["struct"]/stats["total"]*100 if stats["total"] else 0, # Handle ZeroDivisionError
        "final/full_correct_accuracy": stats["full"]/stats["total"]*100 if stats["total"] else 0, # Handle ZeroDivisionError
        "final/semantic_accuracy": (stats["sem_sum"]/stats["sem_cnt"]*100)
                                   if stats["sem_cnt"] else 0.0,
        "final/avg_generation_time": sum(stats["gtimes"])/len(stats["gtimes"]) if stats["gtimes"] else 0, # Handle ZeroDivisionError
        "final/avg_validation_time": sum(stats["vtimes"])/len(stats["vtimes"]) if stats["vtimes"] else 0, # Handle ZeroDivisionError
        "final/avg_attempts":       sum(stats["atts"])/len(stats["atts"]) if stats["atts"] else 0, # Handle ZeroDivisionError
