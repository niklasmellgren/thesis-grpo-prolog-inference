...
def semantic_similarity_reward(completions, answer, **kwargs) -> list[float]:
    """
    Computes a reward based on the semantic similarity between the generated Prolog code
    and the reference code using the all-MiniLM-L6-v2 sentence transformer model.
    """
    ...
    for model_code, ref_code in zip(extracted_responses, answer):
        ...
        # Get embeddings for both pieces of code
        ...
        # Scale the similarity to a reward (e.g., cosine similarity in [0,1] to a reward up to 2.0)
        reward = max(0.5, cosine_sim * 2.0)
